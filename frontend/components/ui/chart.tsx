...
const ChartStyle = ({ id, config }: { id: string; config?: ChartConfig }) => {
  // Ensure config is defined with a default empty object
...
export const ChartTooltip = React.forwardRef<HTMLDivElement, ChartTooltipProps>(
  ({ className, children, ...props }, ref) => {
    return (
      <div
        className={cn(
          "bg-zinc-800 border border-zinc-700 rounded-md shadow-md p-2",
          className,
        )}
        {...props}
        ref={ref}
...
>(({ className, content, ...props }, ref) => {
  return (
    <div className={cn("text-white", className)} {...props} ref={ref}>
      {content}
    </div>
  );
});
